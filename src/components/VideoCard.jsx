...
function VideoCard() {
    ...
    useEffect (() => {
      const fetchData = () => {
       ...
       getData.then((data) => setVideos(data[0]))
        ...
        <div className='flex gap-3 p-2 cursor-pointer font-semibold rounded'>
          <div className='md:w-16 md:h-16 w-10 h-10'>
            <Link to='/'>
              ...
              className='rounded'
              src={videos.postedBy?.image} alt="Profile Photo" />
            </Link>
          </div>
          <div>
            <Link to='/'>
            <div className='flex items-center gap-2'>
                <p className='flex gap-2 items-center md:text-md font-bold text-primary'>{videos.userId}
